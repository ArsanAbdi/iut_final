...
-- deuxième méthode --
	SELECT idTransaction, idPosteDepense, montant, SUM(montant) OVER (ORDER BY idTransaction) AS cumul_dep FROM depense;
...
-- deuxième méthode --
	SELECT idTransaction, ABS(montant - (SELECT AVG(montant) FROM recette)) AS "montant - moyenne" FROM recette;
