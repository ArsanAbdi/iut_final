 ...
 *Le programme demande la saisie d'un double compris entre 0 et 20 (la moyenne au baccalauréat), puis, selon le cas, affiche :
    ...
    •."moyenne incorrecte" lorsque la valeur saisie est négative ou strictement supérieure à 20 ; 1

    •."refusé" lorsque la valeur saisie est strictement inférieure à 8 ; 1

    •."doit passer l'oral de rattrapage" lorsque la valeur saisie est comprise entre 8 inclus et 10 exclus ; 1
...
public class Moyenne {
	public static void main(String[]args) {
		...
		if (laMoyenne < 20 && laMoyenne > 0) {
				...
				else if (laMoyenne >= 10 && laMoyenne < 12)
				System.out.println("mention passable t'étais à deux doigts de la 					mention");

				else if (laMoyenne >= 12 && laMoyenne < 14)
				System.out.println("assez bien bon courage pour la suite");

				else if (laMoyenne >= 14 && laMoyenne < 16)
				System.out.println("mention bien t'es bon");

				else {
				System.out.println("mention très bien ! Bien joué");
				     }
					
							}
		else 
		System.out.println("erreur c'est pas une moyenne ça");
